...
export default async function ClientInvoicesPage() {
  ...
  const { data: invoices } = await supabase
    .from('invoices').select('id, invoice_number, total_ttc, status, due_at')
    .eq('client_id', access.client_id).order('created_at', { ascending: false });
        ...
        {invoices && invoices.length > 0 ? (
          <div className="overflow-x-auto"><table className="w-full text-left text-sm">
            <thead><tr className="border-b border-slate-100 text-xs uppercase tracking-wider text-slate-400">
              <th className="px-5 py-3.5">N°</th><th className="px-5 py-3.5 text-right">TTC</th>
              <th className="px-5 py-3.5">Échéance</th><th className="px-5 py-3.5">Statut</th>
            </tr></thead>
            <tbody className="divide-y divide-slate-50">
              {invoices.map(inv => { const st = ST[inv.status] ?? ST.brouillon; return (
                <tr key={inv.id} className="hover:bg-slate-50/60">
                  <td className="px-5 py-3.5 font-mono text-xs">{inv.invoice_number}</td>
                  <td className="px-5 py-3.5 text-right tabular-nums font-medium">{inv.total_ttc ? new Intl.NumberFormat('fr-FR').format(inv.total_ttc) + ' F' : '—'}</td>
                  <td className="px-5 py-3.5 text-slate-600">{inv.due_at ?? '—'}</td>
                  <td className="px-5 py-3.5"><span className={"rounded-lg px-2 py-0.5 text-xs font-semibold " + st.c}>{st.l}</span></td>
                </tr>); })}
            </tbody>
          </table></div>
