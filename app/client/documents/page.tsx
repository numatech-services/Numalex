...
export default async function ClientDocumentsPage() {
  ...
  const { data: matters } = await supabase.from('matters').select('id').eq('client_id', access.client_id);
  const ids = (matters ?? []).map(m => m.id);
  let docs: Array<{ id: string; title: string; doc_type: string; file_url: string | null; file_size: number | null; created_at: string; matter: { title: string } | null }> = [];
  if (ids.length > 0) {
    const { data } = await supabase.from('documents')
      .select('id, title, doc_type, file_url, file_size, created_at, matter:matters!documents_matter_id_fkey(title)')
      .in('matter_id', ids).order('created_at', { ascending: false }).limit(50);
    docs = data ?? [];
  }
          ...
          <div className="divide-y divide-slate-50">
            {docs.map(d => (
              <div key={d.id} className="flex items-center gap-4 px-5 py-4 hover:bg-slate-50/60">
                ...
                {d.file_url && <a href={d.file_url} target="_blank" rel="noopener noreferrer" className="shrink-0 rounded-lg bg-slate-100 px-3 py-1.5 text-xs font-medium text-slate-700 hover:bg-slate-200">Télécharger</a>}
              </div>
            ))}
          </div>
