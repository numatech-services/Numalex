...
export default async function FacturesPage() {
  ...
  const { data: invoices, count }: { data: any[] | null, count: number | null } = await supabase
    .from('invoices')
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        {invoices && invoices.length > 0 ? (
                  ...
                  return (
                    <tr key={inv.id} className="group transition-colors hover:bg-slate-50/60">
                      ...
                      <td className="px-5 py-4">
                        <span className={`inline-flex rounded-lg px-2 py-0.5 text-xs font-medium ring-1 ring-inset ${st.cls}`}>
                          {st.label}
                        </span>
                      </td>
                      <td className="px-5 py-4 text-right">
                        <Link href={`/dashboard/factures/${inv.id}`} className="inline-flex h-8 items-center rounded-lg border border-slate-200 bg-white px-3 text-xs font-medium text-slate-600 hover:bg-slate-50">
                          Modifier
                        </Link>
                      </td>
                    </tr>
          ...
          <div className="flex flex-col items-center gap-4 px-6 py-20 text-center">
            ...
            <Link href="/dashboard/factures/nouvelle" className="mt-2 inline-flex h-9 items-center rounded-lg bg-slate-900 px-4 text-sm font-medium text-white shadow-sm hover:bg-slate-800">
              Créer une facture
            </Link>
          </div>
        )}
...
function fmtCFA(n: number | null) { 
  return n == null ? '—' : new Intl.NumberFormat('fr-FR', { maximumFractionDigits: 0 }).format(n) + ' F CFA'; 
}
