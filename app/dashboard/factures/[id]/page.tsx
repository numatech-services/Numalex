...
export default async function EditInvoicePage({ params }: { params: { id: string } }) {
  ...
  const [{ data: invoice }, { data: clients }, { data: matters }] = await Promise.all([
    supabase.from('invoices').select('id, invoice_number, amount_ht, tva_rate, status, client_id, matter_id, issued_at, due_at, paid_at, notes').eq('id', params.id).eq('cabinet_id', profile.cabinet_id).single(),
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-8 sm:px-6">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
        ...
        <div className="mt-6"><InvoiceForm initialData={invoice ?? undefined} clients={clients ?? []} matters={matters ?? []} /></div>
      </div>
    </div>
  );
}
