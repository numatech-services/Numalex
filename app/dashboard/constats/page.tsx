          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
              <thead><tr className="border-b border-slate-100 text-xs uppercase tracking-wider text-slate-400">
                ...
                <th className="px-5 py-3.5">Lieu</th><th className="px-5 py-3.5">Statut</th>
              </tr></thead>
              <tbody className="divide-y divide-slate-50">
                {reports.map(r => (
                  <tr key={r.id} className="hover:bg-slate-50/60">
                    ...
                    <td className="px-5 py-3"><span className="rounded-lg bg-violet-50 px-2 py-0.5 text-xs font-medium text-violet-700">{RT[r.report_type] ?? r.report_type}</span></td>
                    <td className="px-5 py-3 text-slate-600">{r.location ?? '—'}</td>
                    <td className="px-5 py-3">{r.served ? <span className="rounded-lg bg-emerald-50 px-2 py-0.5 text-xs font-medium text-emerald-700">Signifié</span> : <span className="rounded-lg bg-slate-100 px-2 py-0.5 text-xs font-medium text-slate-600">En cours</span>}</td>
                  </tr>
                ))}
              </tbody>
            </table>
